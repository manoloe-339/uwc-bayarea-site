...
import { countAlumni, type AlumniFilters } from "@/lib/alumni-query";
...
export default async function EmailPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  // Inherit the alumni search filter shape, but force subscription=subscribed for safety.
  ...
  const recipientCount = await countAlumni(filters);
