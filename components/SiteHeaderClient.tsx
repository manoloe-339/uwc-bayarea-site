...
/** Optional per-event nav item, populated server-side from
 *  events.feature_in_nav via lib/nav-featured-event.ts.
 *  Callers should fetch and pass this in; when null, the item is omitted. */
export type FeaturedNavItem = { label: string; href: string };
...
export default function SiteHeader({
  ...
  active?: NavKey;
  featuredEvent?: FeaturedNavItem | null;
}) {
  // Sign Up is promoted to a standalone CTA on mobile; the rest live
  // in the hamburger drawer. Built inline so featuredEvent (which
  // depends on live DB state) can be injected without hoisting a
  // module-scope const.
  const secondaryLinks: { key: NavKey; label: string; href: string }[] = [
    { key: "home", label: "Home", href: "/" },
    { key: "events", label: "Events", href: "/events" },
    ...(featuredEvent
      ? [{ key: "featured" as NavKey, label: featuredEvent.label, href: featuredEvent.href }]
      : []),
    ...
    >
      <div className="max-w-[1200px] mx-auto flex items-center gap-3 px-3 sm:px-7 py-[14px] sm:py-[18px]">
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          {featuredEvent && (
            <NavLink href={featuredEvent.href} active={active === "featured"}>
              {featuredEvent.label}
            </NavLink>
          )}
