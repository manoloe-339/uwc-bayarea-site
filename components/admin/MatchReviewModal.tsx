"use client";

import { useEffect, useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";

type AlumniHit = {
  id: number;
  first_name: string | null;
  last_name: string | null;
  email: string | null;
  uwc_college: string | null;
  grad_year: number | null;
  photo_url: string | null;
};

type Props = {
  attendeeId: number;
  currentAlumniId: number | null;
  stripeName: string | null;
  stripeEmail: string | null;
  matchReason: string | null;
  onClose: () => void;
};

export function MatchReviewModal({
  attendeeId,
  currentAlumniId,
  stripeName,
  stripeEmail,
  matchReason,
  onClose,
}: Props) {
  const [query, setQuery] = useState(stripeName ?? stripeEmail ?? "");
  const [results, setResults] = useState<AlumniHit[]>([]);
  const [picked, setPicked] = useState<number | null>(currentAlumniId);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const abortRef = useRef<AbortController | null>(null);
  const [, startTransition] = useTransition();
  const router = useRouter();

  useEffect(() => {
    const q = query.trim();
    if (q.length < 2) {
      setResults([]);
      return;
    }
    abortRef.current?.abort();
    const ac = new AbortController();
    abortRef.current = ac;
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/ticket-events/alumni-search?q=${encodeURIComponent(q)}`, {
          signal: ac.signal,
        });
        if (!res.ok) return;
        const { results } = (await res.json()) as { results: AlumniHit[] };
        setResults(results);
      } catch {
        // aborted
      }
    }, 180);
    return () => {
      clearTimeout(t);
      ac.abort();
    };
  }, [query]);

  const confirm = async () => {
    setError(null);
    setSubmitting(true);
    try {
      const res = await fetch(`/api/ticket-events/attendees/${attendeeId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ alumni_id: picked }),
      });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error ?? "Failed to update match");
      }
      startTransition(() => router.refresh());
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed");
    } finally {
      setSubmitting(false);
    }
  };

  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-[1px] p-4">
      <div className="bg-white rounded-[12px] shadow-xl p-6 w-full max-w-[560px] max-h-[90vh] overflow-y-auto">
        <h2 className="font-sans font-bold text-navy text-lg mb-1">Match to alumni</h2>
        <p className="text-xs text-[color:var(--muted)] mb-4">
          {matchReason ? matchReason : "Choose who this purchase belongs to."}
        </p>

        <div className="mb-5 p-3 bg-ivory-2 border border-[color:var(--rule)] rounded text-sm">
          <div className="text-[11px] tracking-[.22em] uppercase font-bold text-[color:var(--muted)] mb-1">
            Stripe payment
          </div>
          <div className="font-semibold text-navy">{stripeName ?? "—"}</div>
          <div className="text-xs text-[color:var(--muted)]">{stripeEmail ?? "—"}</div>
        </div>

        <label className="block mb-3">
          <span className="block text-[11px] tracking-[.22em] uppercase font-bold text-navy mb-1">
            Search alumni
          </span>
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="w-full border border-[color:var(--rule)] rounded px-3 py-2 text-sm bg-white"
            autoFocus
          />
        </label>

        {results.length === 0 ? (
          query.trim().length < 2 ? (
            <p className="text-xs text-[color:var(--muted)] mb-4">Type to search…</p>
          ) : (
            <p className="text-xs text-[color:var(--muted)] mb-4">No matches.</p>
          )
        ) : (
          <ul className="mb-4 border border-[color:var(--rule)] rounded divide-y divide-[color:var(--rule)] max-h-[260px] overflow-y-auto">
            {results.map((r) => (
              <li key={r.id}>
                <label className="flex items-center gap-3 px-3 py-2 hover:bg-ivory-2 cursor-pointer text-sm">
                  <input
                    type="radio"
                    name="match-pick"
                    checked={picked === r.id}
                    onChange={() => setPicked(r.id)}
                    className="shrink-0"
                  />
                  {r.photo_url ? (
                    <img src={r.photo_url} alt="" className="w-8 h-8 rounded-full object-cover bg-ivory-2 border border-[color:var(--rule)]" />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-ivory-2 border border-[color:var(--rule)] flex items-center justify-center text-[color:var(--muted)] text-xs font-sans font-bold">
                      {(r.first_name?.[0] ?? "?").toUpperCase()}
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold text-navy truncate">
                      {[r.first_name, r.last_name].filter(Boolean).join(" ") || r.email}
                    </div>
                    <div className="text-xs text-[color:var(--muted)] truncate">
                      {r.uwc_college ?? "—"}{r.grad_year ? ` '${String(r.grad_year).slice(-2)}` : ""} · {r.email}
                    </div>
                  </div>
                </label>
              </li>
            ))}
          </ul>
        )}

        <label className="flex items-center gap-2 text-sm mb-4">
          <input
            type="radio"
            name="match-pick"
            checked={picked === null}
            onChange={() => setPicked(null)}
          />
          Don&rsquo;t match — keep as unmatched
        </label>

        {error && <div className="mb-3 text-sm text-red-700">{error}</div>}

        <div className="flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm text-[color:var(--muted)] hover:text-navy"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={confirm}
            disabled={submitting}
            className="bg-navy text-white px-5 py-2 rounded text-sm font-semibold disabled:opacity-60"
          >
            {submitting ? "Saving…" : "Confirm match"}
          </button>
        </div>
      </div>
    </div>
  );
}
